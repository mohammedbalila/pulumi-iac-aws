...
const dbName = `${appName.replace(/-/g, "_")}_staging`;
...
const ecrRepository = new ECRRepository(
    `${appName}-staging-${APPLICATION_CONSTANTS.RESOURCE_TYPES.ECR_REPOSITORY}`,
    {
        name: appName,
        environment: "staging",
        retentionDays: ECR_CONSTANTS.RETENTION_DAYS.STAGING,
...
const githubActionsRole = new GitHubActionsRole(
    `${appName}-staging-${APPLICATION_CONSTANTS.RESOURCE_TYPES.GITHUB_ACTIONS_ROLE}`,
    {
        ...
        githubBranches: GITHUB_CONSTANTS.BRANCHES.STAGING,
        githubEnvironments: GITHUB_CONSTANTS.ENVIRONMENTS.STAGING,
...
const networking = new Networking(`${appName}-staging`, {
    name: appName,
    environment: "staging",
    cidrBlock: NETWORKING_CONSTANTS.CIDR_BLOCKS.STAGING,
    availabilityZoneCount: NETWORKING_CONSTANTS.AVAILABILITY_ZONES.STAGING,
...
const database = new Database(`${appName}-staging-db`, {
    ...
    allocatedStorage: DATABASE_CONSTANTS.DEFAULT_STORAGE.STAGING,
    maxAllocatedStorage: DATABASE_CONSTANTS.MAX_STORAGE.STAGING,
});
...
const ecrImageUri = pulumi
    ...
    .apply(([account, region, name]) => {
        ...
        return `${account.accountId}.dkr.ecr.${region.name}.amazonaws.com/${name}-staging:latest`;
    });

// Create App Runner service with ECR source
const appService = new AppRunnerService(
    `${appName}-staging-${APPLICATION_CONSTANTS.RESOURCE_TYPES.APP_RUNNER_SERVICE}`,
    {
        name: appName,
        environment: "staging",
        ...
        environmentVariables: {
            ...
            [APPLICATION_CONSTANTS.ENV_VARS.LOG_LEVEL]: APPLICATION_CONSTANTS.LOG_LEVELS.INFO,
            [APPLICATION_CONSTANTS.ENV_VARS.FEATURE_FLAGS]: JSON.stringify(FEATURE_FLAGS.STAGING),
        },

        // Scaling configuration for staging
        maxConcurrency: APP_RUNNER_CONSTANTS.SCALING.STAGING.MAX_CONCURRENCY,
        maxSize: APP_RUNNER_CONSTANTS.SCALING.STAGING.MAX_SIZE,
        minSize: APP_RUNNER_CONSTANTS.SCALING.STAGING.MIN_SIZE,
...
const monitoring = new Monitoring(
    `${appName}-staging-${APPLICATION_CONSTANTS.RESOURCE_TYPES.MONITORING}`,
    {
        name: appName,
        environment: "staging",
...
if (enableWaf) {
    const waf = new AppWaf(`${appName}-staging-${APPLICATION_CONSTANTS.RESOURCE_TYPES.WAF}`, {
        name: appName,
        environment: "staging",
        resourceArn: appService.getServiceArn(),
        rateLimit: WAF_CONSTANTS.RATE_LIMITS.STAGING,
    });
...
if (enableCloudFront) {
    ...
    const dist = new aws.cloudfront.Distribution(
        `${appName}-staging-${APPLICATION_CONSTANTS.RESOURCE_TYPES.CLOUDFRONT}`,
        {
            enabled: true,
            isIpv6Enabled: true,
            origins: [
